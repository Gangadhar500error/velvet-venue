...
interface FilterPanelProps {
  ...
  onChange: (key: keyof CustomerFilters, value: string) => void;
  onApply: () => void;
  onReset: () => void;
  onClose: () => void;
  onSave?: () => void;
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#9CA3AF]">
                    {activeCount > 0 ? `${activeCount} active filter(s)` : "Refine the customer list"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <Field label="Min Bookings">
                <input
                  type="number"
                  min={0}
                  value={filters.bookingsMin}
                  onChange={(e) => onChange("bookingsMin", e.target.value)}
                  placeholder="e.g. 5"
                  className={inputClass}
                />
              </Field>
