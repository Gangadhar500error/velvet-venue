...
import { VenueOwnerFilters } from "../types";
import { cityOptions } from "../data";

interface FilterPanelProps {
  open: boolean;
  filters: VenueOwnerFilters;
  onChange: (key: keyof VenueOwnerFilters, value: string) => void;
  onApply: () => void;
  onReset: () => void;
  onClose: () => void;
  onSave?: () => void;
        ...
        >
          <div className="bg-white border border-[#E8EAF0] rounded-[14px] shadow-[0_1px_2px_rgba(16,24,40,0.04)] p-4 md:p-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#9CA3AF]">
                    {activeCount > 0 ? `${activeCount} active filter(s)` : "Refine the venue owner list"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <Field label="Min Assigned Businesses">
                <input
                  type="number"
                  min={0}
                  value={filters.businessesMin}
                  onChange={(e) => onChange("businessesMin", e.target.value)}
                  placeholder="e.g. 2"
                  className={inputClass}
                />
              </Field>
