...
function enrich(
  ...
  return {
    ...
    venues: [],
    overview: {
      businessProfilesCount: Math.max(assignedBusinesses, linkedBusinesses.length),
      venuesCount: Math.max(totalVenues, linkedBusinesses.length),
      totalBookings: sampleBookings.length,
      revenue: sampleBookings.reduce((sum, b) => sum + b.amount, 0),
      pendingPayments: 0,
      completedEvents: sampleBookings.filter((b) => b.status === "completed").length,
      upcomingEvents: sampleBookings.filter((b) => b.status === "upcoming").length,
    },
    ...base,
    name,
